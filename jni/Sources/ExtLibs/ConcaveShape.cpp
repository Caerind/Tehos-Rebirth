...
namespace detail
{
	...
	: mUserVertex(nullptr)
	, mPosition(x, y)
	, mSurroundingTriangle(std::make_shared<OptTriangleIterator>())
#ifndef NDEBUG
	...
	, mRemainingVertices()
	, mAdjacentTriangles(std::make_shared<OptTriangleItrArray>())
	, mFlagged(false)
	{
	}
	...
	Circle computeCircumcircle(const AdvancedTriangle& triangle)
	{
		...
		sf::Vector2f u = p - at(triangle, 0);
		sf::Vector2f v = sf::Vector2f(-u.y, u.x);
		sf::Vector2f x = q - at(triangle, 0);
		sf::Vector2f w = sf::Vector2f(-x.y, x.x);
        ...
        sf::Vector2f y = intersection - at(triangle, 0);
		return Circle(intersection, y.x * y.x + y.y * y.y);
	}
	...
	{
		for (const AdvancedEdge& constrainedEdge : constrainedEdges)
		{
	...
	bool isVertexInSection(const AdvancedVertex& vertex, sf::Vector2f center, sf::Vector2f corner1, sf::Vector2f corner2)
	{
        ...
        sf::Vector2f a = corner1 - center, b = vertex.getPosition() - center;
        sf::Vector2f c = corner2 - center, d = vertex.getPosition() - center;
	...
	{
	    sf::Vector2f a = corner2 - corner1, b = vertex.getPosition() - corner1;
		return a.x * b.y - a.y * b.x >= 0.f;
	}
		...
		const EdgeSet& constrainedEdges)
	{
		...
		sf::Vector2f a = at(*second, disjointCornerIndices.second) - circle.midPoint;
		sf::Vector2f b = at(*first, disjointCornerIndices.first) - circle2.midPoint;
		if (!(circle.squaredRadius == InfiniteRadius && circle2.squaredRadius == InfiniteRadius)				// (0)
		 && a.x * a.x + a.y * a.y < circle.squaredRadius	// (1)
		 && b.x * b.x + b.y * b.y < circle2.squaredRadius)	// (2)
		{
			changeEdgeSituation(triangles, first, second, boundaryTriangle, constrainedEdges, sharedCornerIndices1, sharedCornerIndices2, disjointCornerIndices);
			return true;
		}
	...
	void setBoundaryPositions(const VertexCtr& allVertices, AdvancedTriangle& boundaryTriangle)
	{
		...
		float maxCoord = 0.f;
		for (const AdvancedVertex& vertex : allVertices)
		{
			sf::Vector2f position = vertex.getPosition();
            if (position.x < 0.f) position.x *= -1.f;
            if (position.y < 0.f) position.y *= -1.f;
			maxCoord = std::max(maxCoord, position.x);
			maxCoord = std::max(maxCoord, position.y);
		}
...
void ConcaveShape::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	for (const sf::ConvexShape& shape : mOutlineShapes)
...
void ConcaveShape::ensureOutlineUpdated() const
{
	...
	for (std::size_t i = 0; i < mPoints.size(); ++i)
	{
		...
		sf::Vector2f perpendicular = 0.5f * mOutlineThickness * sf::Vector2f(-direction.y, direction.x) / std::sqrt(direction.y * direction.y + direction.x * direction.x);
        line.setFillColor(mOutlineColor);
		...
		// Add shapes
		const std::size_t size = circle.getPointCount();
        sf::ConvexShape convexShape;
        convexShape.setFillColor(circle.getFillColor());
        convexShape.setOutlineColor(circle.getOutlineColor());
        convexShape.setOutlineThickness(circle.getOutlineThickness());
        convexShape.setPointCount(size);
        convexShape.setPosition(circle.getPosition());
        convexShape.setRotation(circle.getRotation());
        convexShape.setScale(circle.getScale());
        convexShape.setOrigin(circle.getOrigin());
        convexShape.setTexture(circle.getTexture());
        convexShape.setTextureRect(circle.getTextureRect());
        for (std::size_t i = 0; i < size; ++i)
        {
            convexShape.setPoint(i, circle.getPoint(i));
        }

		mOutlineShapes.push_back(convexShape);
